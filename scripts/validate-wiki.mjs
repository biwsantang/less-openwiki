...
if (!(await isDirectory(wikiRoot))) {
  ...
  for (const artifact of forbiddenArtifacts) {
    if (await exists(path.join(wikiRoot, artifact))) {
      errors.push(
        `${path.relative(root, wikiRoot)}: forbidden lifecycle artifact ${artifact}.`,
      );
    }
  }
...
function parseOptions(args) {
  const options = { root: process.cwd(), wikiDir: "wiki" };
