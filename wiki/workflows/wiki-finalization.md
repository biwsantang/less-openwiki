...
2. **Validate the skipped-page-snapshot contract.** A page job may be `skipped`
   ...
   paths fail the run with _"Every skipped OpenWiki page job requires its
   original page snapshot before finish."_ before any wiki bytes are touched.
...
3. **Record the pre-finish source state and build per-page producers.**
   ...
   `sourceChangedBeforeFinish` before any finalization runs. This is a _recording_
