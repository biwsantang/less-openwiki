...
For provider setup and credentials see [Model Providers](/wiki/concepts/model-providers.md) and [Configuration](/wiki/operations/configuration.md); for the repository init/update flow that bypasses the shared agent graph see [Repository Generation](/wiki/workflows/repository-generation.md).
